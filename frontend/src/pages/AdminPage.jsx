...
export default function AdminPage() {
  ...
  const [form, setForm] = useState({
    candidate_name: "", candidate_id: "", candidate_email: "",
    candidate_phone: "", position: "", company: "",
  });
  ...
  const [userForm, setUserForm] = useState({ username: "", password: "", display_name: "", role: "creator" });
  ...
  async function createEval(e) {
    ...
    try {
      ...
      setCreatedLink(link);
      setForm({ candidate_name: "", candidate_id: "", candidate_email: "", candidate_phone: "", position: "", company: "" });
      await loadList();
    } finally {
      setCreating(false);
    }
  ...
  async function handleCreateUser(e) {
    ...
    try {
      ...
      setUserForm({ username: "", password: "", display_name: "", role: "creator" });
      await loadUsers();
    } finally {
          ...
          <div className="container" style={{ maxWidth: 380 }}>
            <div className="card" style={{ padding: "32px 28px" }}>
              <div style={{ textAlign: "center", marginBottom: 24 }}>
                ...
                <p style={{ color: "var(--text3)", fontSize: 13, marginTop: 6 }}>Ingresa tus credenciales</p>
              </div>
              <form onSubmit={login}>
                <div className="field">
                  <label>Usuario</label>
                ...
                <div className="field">
                  ...
                  />
                  {authError && (
                    <p style={{ color: "var(--danger)", fontSize: 12, marginTop: 6 }}>Usuario o contraseña incorrectos</p>
                  )}
                </div>
                <button className="btn btn-primary btn-full" type="submit" disabled={authLoading || !username || !password}>
  ...
  return (
    <div style={{ minHeight: "100dvh", background: "var(--bg)" }}>
      <div className="app-header">
        ...
        <button className="btn btn-ghost" style={{ fontSize: 12, padding: "6px 14px" }} onClick={logout}>
          Salir
        </button>
      </div>
      ...
      {userRole === "admin" && (
        <div style={{ borderBottom: "1px solid var(--border)", background: "white", padding: "0 16px" }}>
                ...
                <form onSubmit={createEval}>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                    ...
                    <div className="field">
                      ...
                      <input className="input" value={form.position} placeholder="Analista de Recursos Humanos"
            ...
            <>
                ...
                <form onSubmit={handleCreateUser}>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                    <div className="field">
                      ...
                      <input className="input" value={userForm.username} placeholder="rrhh"
                        onChange={e => setUserForm(f => ({ ...f, username: e.target.value }))} />
                    </div>
                    <div className="field">
                      <label>Nombre para mostrar</label>
                      <input className="input" value={userForm.display_name} placeholder="María López"
                        onChange={e => setUserForm(f => ({ ...f, display_name: e.target.value }))} />
                    </div>
                    <div className="field">
                      <label>Contraseña *</label>
                      <input className="input" type="password" value={userForm.password} placeholder="••••••••"
                  ...
                  {userError && (
                    <p style={{ color: "var(--danger)", fontSize: 12, marginTop: 8 }}>{userError}</p>
                  )}
                  <button className="btn btn-primary" type="submit" disabled={creatingUser} style={{ marginTop: 8 }}>
              ...
              <div className="card" style={{ padding: 0, overflow: "hidden" }}>
                <div style={{ padding: "16px 20px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center" }}>
                  ...
                  <button className="btn btn-ghost" style={{ fontSize: 12, padding: "6px 14px" }} onClick={loadUsers}>
                      ...
                      <tbody>
                        {adminUsers.map(u => (
                          <tr key={u.id}>
                            ...
                            <td style={{ fontSize: 12, color: "var(--text3)" }}>
                              {u.created_at ? new Date(u.created_at).toLocaleDateString("es", { day: "2-digit", month: "short", year: "numeric" }) : "—"}
                            </td>
                            <td>
                              {u.id !== currentUserId ? (
                                <button
                                ...
                                <span style={{ fontSize: 11, color: "var(--text4)" }}>Tú</span>
                              )}
                            </td>
                          </tr>
                        ))}
